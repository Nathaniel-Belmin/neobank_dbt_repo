...
baseline AS (
    SELECT
        (SELECT APPROX_QUANTILES(SAFE_DIVIDE(num_transactions, lifetime_days), 100)[OFFSET(50)] FROM user_transactions, user_lifetime) AS median_transactions
),
...
engagement_status AS (
    SELECT
        ...
        ROUND(SAFE_DIVIDE(num_transactions, lifetime_days), 2) as engagement_rate,
        CASE
            WHEN SAFE_DIVIDE(ut.num_transactions, ul.lifetime_days) > (SELECT median_transactions FROM baseline) THEN 'Engaged'
            ELSE 'Not Engaged'
